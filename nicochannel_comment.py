...
if __name__=='__main__' :
    ...
    if '/live/' in video_url:
        video_url = video_url.replace('/live/','/video/')
    #fc_site_id = args.fc_site_id

    # Check Video URL
    if re.compile('video/[0-9A-Za-z]+$').search(video_url):
...
for data in ch_fcid_request.json().get("data", {}).get("content_providers", []):
    if data["domain"] == f'{video_url}'.split('/video')[0]:
...
headers = {'User-Agent': ua, 'Accept': 'application/json, text/plain, */*', 'Accept-Language': 'ja', 'Accept-Encoding': 'gzip, deflate, br', 
           'fc_site_id': '105', 'fc_use_device': 'null', 'Origin': f'https://{parse.netloc}', 'DNT': '1', 'Connection': 'keep-alive', 
           'Referer': f'https://{parse.netloc}/', 'Sec-Fetch-Dest': 'empty', 'Sec-Fetch-Mode': 'cors', 'Sec-Fetch-Site': 'same-site', 
           'TE': 'trailers', 'accept': 'application/json, text/plain, */*', 'accept-language': 'ja,en-US;q=0.9,en;q=0.8',
           'fc_site_id': fc_site_id}
try :
    video_data_req = requests.get(f'https://api.{parse.netloc}/fc/video_pages/' + vid, headers=headers)
...
if not re.compile(r'^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$').search(oldest_time) :
    print('ERROR! / Could not get time information.')
    sys.exit(1)
oldest_time = re.sub(r' (\d{2}:\d{2}):\d{2}$', 'T\\1:00.000Z', oldest_time)

title = str(video_data.get('data', {}).get('video_page', {}).get('title'))


# Get User Access Token
headers = {'User-Agent': ua, 'Accept': 'application/json, text/plain, */*', 'Accept-Language': 'ja', 'Origin': f'https://{parse.netloc}', 'Connection': 'keep-alive', 'Referer': f'https://{parse.netloc}/', 'Sec-Fetch-Dest': 'empty', 'Sec-Fetch-Mode': 'cors', 'Sec-Fetch-Site': 'same-site'}
user_token_req = requests.get(f'https://api.{parse.netloc}/fc/video_pages/' + vid + '/comments_user_token', headers=headers)
try :
...
# Get Comments
headers = {'User-Agent': ua, 'Accept': 'application/json, text/plain, */*', 'Accept-Language': 'ja', 'Origin': f'https://{parse.netloc}', 'Connection': 'keep-alive', 'Referer': f'https://{parse.netloc}/', 'Sec-Fetch-Dest': 'empty', 'Sec-Fetch-Mode': 'cors', 'Sec-Fetch-Site': 'cross-site'}
...
while True :
    comments_req = requests.post(f'https://comm-api.sheeta.com/messages.history?limit=120&oldest={oldest_time}&sort_direction=asc', headers=headers, json=post_json)
    try :
    ...
    if len(comments_req_data) == 0 or len(comments_req_data) == 1 :
        def sanitize_xml(content):
            # 允许的字符：制表符（9）、换行（10）、回车（13）、以及 32 及以上合法 Unicode
            allowed = set(range(32, 0x10FFFF)) | {9, 10, 13}
            return ''.join(c if ord(c) in allowed else '?' for c in content)

        # 生成 XML 并清理
        xml_bytes = ET.tostring(packet, encoding='utf-8')
        xml_clean = sanitize_xml(xml_bytes.decode('utf-8'))
        tree = minidom.parseString(xml_clean)
